...
class YearToDateLowAnalyzer:
    ...
    def get_detailed_stock_info(self, stock_code: str) -> Optional[Dict]:
        ...
        try:
            ...
            # 安値からの回復率
            max_drawdown = (
                ((ytd_low - ytd_high) / ytd_high) * 100 if ytd_high > 0 else 0
            )

            # 技術指標の計算
            sma_20 = (
                hist["Close"].rolling(window=20).mean().iloc[-1]
                if len(hist) >= 20
                else current_price
            )
            sma_50 = (
    ...
    def filter_recovery_candidates(
        ...
        if "min_recovery_score" in criteria:
            filtered_df = filtered_df[
                filtered_df["recovery_score"] >= criteria["min_recovery_score"]
            ]

        # 安値からの回復率によるフィルタ
        if "min_recovery_from_low" in criteria:
            filtered_df = filtered_df[
                filtered_df["recovery_from_low_pct"]
                >= criteria["min_recovery_from_low"]
            ]

        # PBR によるフィルタ
        if "max_pb_ratio" in criteria:
            pb_mask = (filtered_df["pb_ratio"] != "N/A") & (
                pd.to_numeric(filtered_df["pb_ratio"], errors="coerce")
                <= criteria["max_pb_ratio"]
            )
            filtered_df = filtered_df[pb_mask]

        # 配当利回りによるフィルタ
        if "min_dividend_yield" in criteria:
            div_mask = (filtered_df["dividend_yield"] != "N/A") & (
                pd.to_numeric(filtered_df["dividend_yield"], errors="coerce")
    ...
    def save_analysis_results(
        self, df: pd.DataFrame, filename: str = "ytd_low_analysis.csv"
    ) -> None:
        """
        分析結果をCSVファイルに保存
    ...
    def print_recovery_candidates(self, df: pd.DataFrame, top_n: int = 10) -> None:
        ...
        if "recovery_score" in df.columns:
            ...
            for i, row in top_stocks.iterrows():
                ...
                if "ytd_low" in row:
                    print(
                        f"    年初来安値: {row['ytd_low']:,.0f}円 ({row.get('ytd_low_date', 'N/A')})"
                    )
...
def main():
    ...
    # 結果表示
    analyzer.print_recovery_candidates(
        detailed_df if not detailed_df.empty else basic_df
    )
